...
const verdictDecisions = Object.fromEntries(Object.entries(ACTIONS).map(([k, v]) => [k, v.decision]));
function verdictDecision(code) {
...
async function loadVocabulary() {
    try {
        ...
        if (code !== 0)
            return;
        const vocab = JSON.parse(stdout);
        for (const v of vocab.verdicts ?? []) {
            if (v.code && v.decision)
                verdictDecisions[v.code] = v.decision;
...
const decoTypes = Object.fromEntries(Object.keys(STATE).map((s) => [s, vscode.window.createTextEditorDecorationType({
...
function cliPath() {
    return vscode.workspace.getConfiguration("citevahti").get("cliPath") || "citevahti";
}
function projectRoot() {
    const cfg = vscode.workspace.getConfiguration("citevahti").get("root");
    if (cfg)
        return cfg;
...
function runCli(args, env) {
    ...
    const opts = { maxBuffer: 8 << 20, encoding: "utf8" };
    if (env)
        opts.env = { ...process.env, ...env };
    return new Promise((resolve) => {
        (0, child_process_1.execFile)(cliPath(), ["--root", projectRoot(), ...args], opts, (err, stdout, stderr) => resolve({ code: err && typeof err.code === "number" ? err.code : (err ? 1 : 0),
...
async function connectZotero() {
    ...
    const go = await vscode.window.showInformationMessage("Connect Zotero: I'll open the key page (name + write permission pre-filled). " +
        "Click “Save Key”, copy the key, then paste it back here. Do you write to a shared/group library?", "My library only", "Include shared/group");
    if (go !== "My library only" && go !== "Include shared/group")
        return;
    ...
    if (!key || !key.trim())
        return;
    const { code, stdout, stderr } = await runCli(["connect-zotero", "--no-open"], { CITEVAHTI_ZOTERO_WRITE_KEY: key.trim() });
    if (code === 0) {
...
async function runReport() {
    ...
    return JSON.parse(stdout); // exits non-zero when claims need attention; that's fine
}
...
function decorate(report) {
    ...
    const ranges = { accepted: [], needs_support: [], review_needed: [], decision_recorded: [], untestable: [] };
    ...
    for (const s of Object.keys(STATE))
        editor.setDecorations(decoTypes[s], ranges[s]);
}
...
function fitGrade(n) { return n >= 7 ? "Strong" : n >= 4 ? "Moderate" : n >= 1 ? "Weak" : "None"; }
function fitHtml(e) {
    if (!e.fit || e.fit_total == null)
        return ""; // blinded / not yet rated by the human
...
function evidenceHtml(claimId, e) {
    ...
    const rateBtns = SUPPORT_VALUES.map(([v, label], i) => `<button class="ratebtn" ${data} data-value="${v}" title="Record your blind support rating">` +
        `<span class="kbd">${i + 1}</span> ${label}</button>`).join(" ");
    const decideBtns = Object.keys(ACTIONS).map((k) => `<button class="act ${ACTIONS[k].cls}" ${data} data-decision="${verdictDecision(k)}"` +
...
function webviewHtml(report) {
    const nonce = (0, crypto_1.randomBytes)(16).toString("base64");
    const c = report.counts;
    const summary = Object.keys(STATE).map((s) => `<span class="chip ${s}"><b>[${STATE[s].code}]</b> ${STATE[s].label} ${c[s] ?? 0}</span>`).join(" ");
...
async function refresh() {
    ...
    decorate(report);
    if (panel)
        panel.webview.html = webviewHtml(report);
}
function revealInEditor(text) {
    const editor = vscode.window.activeTextEditor;
    if (!editor || !text)
        return;
    ...
    if (i < 0)
        return;
    const range = new vscode.Range(editor.document.positionAt(i), editor.document.positionAt(i + text.trim().length));
    editor.revealRange(range, vscode.TextEditorRevealType.InCenter);
    editor.selection = new vscode.Selection(range.start, range.end);
}
async function decide(claim, cand, rating, decision) {
...
async function rate(claim, cand, rating, value) {
    let ratingId = rating;
    if (!ratingId) {
        const started = await runCli(["claim-support-start", "--claim-id", claim, "--candidate-id", cand]);
        if (started.code !== 0) {
            vscode.window.showErrorMessage(`CiteVahti: ${(started.stderr || started.stdout || "could not start rating").trim().split("\n")[0]}`);
            return;
        }
    ...
    const { code, stdout, stderr } = await runCli(["claim-support-commit-human", "--rating-id", ratingId, "--value", value]);
    if (code !== 0) {
        vscode.window.showErrorMessage(`CiteVahti: ${(stderr || stdout || "rating failed").trim().split("\n")[0]}`);
    }
    else {
        vscode.window.showInformationMessage(`CiteVahti: recorded your support rating "${value}". The AI second opinion is now unblinded.`);
    }
    await refresh();
}
...
async function commit(decisionId) {
    ...
    let diff;
    try {
        diff = JSON.parse(prev.stdout);
    }
    catch {
        vscode.window.showErrorMessage("CiteVahti: could not preview the write.");
        return;
    }
    if (!diff.backend_available) {
        const pick = await vscode.window.showWarningMessage("CiteVahti: Zotero isn't connected yet. Connect it once (one paste) to enable write-back.", "Connect Zotero");
        if (pick === "Connect Zotero")
            await connectZotero();
        return;
    }
...
async function doCommit(decisionId, override, confirmToken) {
    ...
    let txn;
    try {
        txn = JSON.parse(stdout);
    }
    catch {
        vscode.window.showErrorMessage("CiteVahti: write failed (no result).");
        await refresh();
        return;
    }
    if (txn.status === "committed") {
        const keys = (txn.result?.created_keys || []).join(", ");
        const undo = await vscode.window.showInformationMessage(`Added to Zotero${keys ? " (" + keys + ")" : ""}${txn.collection_key ? " in " + txn.collection_key : ""}. ` +
            `Transaction ${txn.transaction_id}.`, "Undo");
        if (undo === "Undo") {
            const u = await runCli(["txn-undo", "--transaction-id", txn.transaction_id, "--json"]);
            let ut = {};
            try {
                ut = JSON.parse(u.stdout);
            }
            catch { /* ignore */ }
        ...
        const o = await vscode.window.showWarningMessage("Couldn't verify the paper isn't already in your Zotero library (search unavailable). Add anyway?", "Override and add");
        if (o === "Override and add") {
            await doCommit(decisionId, true, confirmToken);
            return;
        }
    }
    else {
...
async function proposeRevision(claim, current) {
    const next = await vscode.window.showInputBox({
        prompt: "Suggest a rewrite — review the diff before it's applied",
        value: current, valueSelection: [0, current.length],
    });
    if (next === undefined || next.trim() === "" || next.trim() === current.trim())
        return;
    const { code, stdout, stderr } = await runCli(["claim-propose-revision", "--claim-id", claim, "--text", next.trim()]);
    if (code !== 0)
...
function fileFromManuscriptLocation(location) {
    const raw = String(location || "").split(":")[0].trim();
    return raw || undefined;
}
async function editorForRevision(oldText, location) {
    const active = vscode.window.activeTextEditor;
    if (active?.document.getText().includes(oldText))
        return active;
...
async function backupManuscript(doc) {
    try {
...
async function revertManuscriptEdit() {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        vscode.window.showWarningMessage("CiteVahti: open the manuscript to revert.");
        return;
    }
    ...
    if (!dir || !latest) {
        vscode.window.showWarningMessage("CiteVahti: no manuscript backup found for this file.");
        return;
    }
    const pick = await vscode.window.showWarningMessage(`Restore "${name}" from backup ${latest}? Current contents will be replaced.`, "Restore");
    if (pick !== "Restore")
        return;
...
async function acceptRevision(claim, oldText, newText, location) {
    ...
    if (selected === oldText) {
        ...
        if (second >= 0) {
            const pick = await vscode.window.showWarningMessage("This claim text appears more than once. Select the intended manuscript span, then accept the revision again.", "Reveal first match");
            if (pick === "Reveal first match")
                revealInEditor(oldText);
            return;
        }
    ...
    const { code, stdout, stderr } = await runCli(["claim-accept-revision", "--claim-id", claim, "--expected-text", newText]);
    if (code !== 0) {
        const startOffset = doc.offsetAt(range.start);
        const rollback = new vscode.WorkspaceEdit();
        rollback.replace(doc.uri, new vscode.Range(doc.positionAt(startOffset), doc.positionAt(startOffset + newText.length)), oldText);
...
async function changeReference(claim) {
    ...
    let res;
    try {
        res = JSON.parse(search.stdout);
    }
    catch {
        vscode.window.showErrorMessage("CiteVahti: search failed (no result). Check citevahti.cliPath / NCBI access.");
        return;
    }
    if (res.status !== "ok") {
    ...
    const hits = res.hits || [];
    if (!hits.length) {
        vscode.window.showInformationMessage("CiteVahti: no results for that query.");
        return;
    }
    ...
    for (const p of picks)
        args.push("--record-id", p.recordId);
    const link = await runCli(args);
    if (link.code === 0) {
        let linked = {};
        try {
            linked = JSON.parse(link.stdout);
        }
        catch { /* ignore */ }
        const n = linked.linked ?? picks.length;
        const dup = linked.skipped_duplicates ? ` (${linked.skipped_duplicates} already linked)` : "";
        vscode.window.showInformationMessage(`CiteVahti: linked ${n} candidate(s)${dup} — rate them on the card to set support.`);
    }
    else {
...
async function rejectRevision(claim) {
...
async function verifyClaims() {
    if (!panel) {
        panel = vscode.window.createWebviewPanel("citevahti", "Citation Integrity", vscode.ViewColumn.Beside, { enableScripts: true });
        panel.onDidDispose(() => (panel = undefined));
        panel.webview.onDidReceiveMessage((m) => {
            if (m?.type === "reveal")
                revealInEditor(String(m.text || ""));
            else if (m?.type === "rate")
                rate(String(m.claim), String(m.cand), m.rating ?? null, String(m.value));
            else if (m?.type === "decide")
                decide(String(m.claim), String(m.cand), m.rating ?? null, String(m.decision));
            else if (m?.type === "commit")
                commit(String(m.decisionId));
            else if (m?.type === "propose-revision")
                proposeRevision(String(m.claim), String(m.text || ""));
            else if (m?.type === "accept-revision")
                acceptRevision(String(m.claim), String(m.oldText || ""), String(m.newText || ""), m.location ?? null);
            else if (m?.type === "reject-revision")
                rejectRevision(String(m.claim));
            else if (m?.type === "change-reference")
                changeReference(String(m.claim));
        });
    }
    await refresh();
}
...
function manuscriptIsOpen() {
    return vscode.window.visibleTextEditors.some((e) => e.document.uri.scheme === "file");
}
function openZotero() {
    if (process.platform === "darwin")
        (0, child_process_1.execFile)("open", ["-a", "Zotero"], () => undefined);
    else
        vscode.window.showInformationMessage("Open Zotero Desktop to add references back to Zotero.");
}
...
async function startReview() {
    if (!reviewPanel) {
        reviewPanel = vscode.window.createWebviewPanel("citevahtiReview", "CiteVahti — Start review", vscode.ViewColumn.Beside, { enableScripts: true });
        reviewPanel.onDidDispose(() => (reviewPanel = undefined));
        reviewPanel.webview.onDidReceiveMessage(async (m) => {
            switch (m?.type) {
                case "setup-project": {
                    const r = await runCli(["init"]);
                    vscode.window.showInformationMessage(r.code === 0 ? "CiteVahti: your review workspace is ready." : "CiteVahti: couldn't set up the workspace.");
                    break;
                }
                case "open-zotero":
                    openZotero();
...
function checkRow(ok, optional, title, note, fix) {
...
function reviewHtml(pf, manuscript) {
    const nonce = (0, crypto_1.randomBytes)(16).toString("base64");
    ...
    const rows = [
        checkRow(manuscript, false, "Manuscript open", manuscript ? "A manuscript file is open." : "Open the document you want to check, then refresh."),
        checkRow(pf.project_initialized, false, "CiteVahti ready", pf.project_initialized ? "Your review workspace is set up." : "One-time setup for this folder.", { label: "Set up", type: "setup-project" }),
        checkRow(total > 0, false, "Claims to review", total > 0 ? `${total} claim${total === 1 ? "" : "s"} found · ${withRefs} already have references`
            : "No claims yet — add the sentences you want to check.", { label: "Add a claim", type: "add-claim" }),
        checkRow(pf.zotero.reachable, true, "Zotero running (optional)", pf.zotero.reachable ? `Connected for reading${pf.zotero.version ? " · v" + pf.zotero.version : ""}.`
            : "Only needed to add references back to Zotero.", { label: "Open Zotero", type: "open-zotero" }),
        checkRow(pf.zotero_write_ready, true, "Saving references to Zotero (optional)", pf.zotero_write_ready ? "Ready — accepted references can be saved to Zotero."
            : "Paste a Zotero key once to enable this. You can do it later.", { label: "Connect", type: "connect-zotero" }),
    ].join("");
