...
| [0010](0010-godfile-decomposition.md) | God-file decomposition — staged, characterization-first, write-boundary-led; reconciles a files-blind best-practices essay with reality (registry + service layer already exist; `tools.py` is a facade; lazy imports are load-bearing; panel is `http.server` not FastAPI); risk gradient is coverage not size; PR 0 characterization net → `tools/` split (write last) → panel `dispatch()` route table → `cli.py` | Proposed (builds on 0001, 0007) |
