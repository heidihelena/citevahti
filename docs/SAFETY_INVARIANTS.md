...
- **A model's judgement is never re-asked.** Only a transient no-answer (an unreachable
  endpoint, an unreadable reply, a raised transport error) is retried, with the identical
  prompt; a rating, an abstention, an out-of-vocabulary answer and a truncated reply are
  all returned on the first attempt. Retrying until the answer changes would select on the
  outcome and could manufacture a rating the model never gave
