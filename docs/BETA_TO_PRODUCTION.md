...
|---|---|---|---|---|
| 1 | [`citevahti-eval`](../skills/citevahti-eval/SKILL.md) | **Honest self-measurement** ([ADR-0009](adr/0009-evaluation-and-model-quality.md)): three eval objects — an automatic claim-lexicon eval we run, continuous model rating by *complementary catches* (not agreement), and a pooled Atlas scoreboard later. Not a human-gated release gate. | product-building + reputational | **Built + running.** `eval_lexicon.py` produces real lexical-layer numbers now (support P 0.71 / R 0.67, contradiction P 1.00 / R 0.50, 0 negation leaks), CI-guarded by `test_lexicon_eval.py`. Model rating + Atlas scoreboard = ADR-0009 roadmap. Human ledger retained as optional calibration. |
